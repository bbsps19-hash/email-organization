...
const translations = {
  ko: {
    ...
    geminiRunning: '잠시만요, 기준에 맞는 메일을 찾고 있어요.',
    ...
    geminiDefaultReply: '알겠어요! 기준에 맞는 메일을 골라볼게요.',
  ...
  en: {
    ...
    geminiRunning: 'Hang tight—finding emails that match your criteria.',
    ...
    geminiDefaultReply: 'Got it! I will find emails that match your criteria.',
...
const fixBrokenEncodedWords = (value) => {
  let fixed = value;
  // Remove dangling empty encoded-word tokens like "=?=" only when standalone.
  fixed = fixed.replace(/(^|[\s,])=\?=\s*/g, '$1');
  // Repair encoded-words that have whitespace or a closing quote before the ?= terminator.
...
const repairMojibake = (value) => {
  ...
  for (let i = 1; i < candidates.length; i += 1) {
    ...
    const preferHangul =
      score.hangul > 0 && bestScore.hangul === 0 && score.replacements <= bestScore.replacements + 1;
    if (
      score.replacements < bestScore.replacements ||
      (score.replacements === bestScore.replacements && score.hangul > bestScore.hangul) ||
      preferHangul
...
const windows1252ToBytes = (text) => {
  ...
  for (const ch of text) {
    ...
    const mapped = WINDOWS_1252_CODEPOINTS[code];
    bytes.push(mapped === undefined ? 0x3f : mapped);
  }
